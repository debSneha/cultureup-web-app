...
describe('media fetch', () => {
    ...
    test('should respond with a 200 status code', async () => {
        ...
        await prisma.mediaOnPost.create({
            data: {
                mediaUrl: "url",
                postId: 1,
            }
        ...
        const { status, body } = await request(app).get('/api/post/get-media')
        .set({
        ...
        expect(body[0].mediaUrl).toStrictEqual("url");
    });

    test('should respond with a 400 status code if not authorised', async () => {


        const { status, body } = await request(app).get('/api/post/get-media').set({
